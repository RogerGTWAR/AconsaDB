--Vehiculo Detalles
CREATE TRIGGER ValidarFechasVehiculodDetalles
ON [Vehiculo Detalles]
AFTER INSERT, UPDATE
AS
BEGIN
    DELETE FROM [Vehiculo Detalles]
    WHERE FechaAsignacion > FechaFinAsignacion AND Vehiculo_DetalleID IN (SELECT Vehiculo_DetalleID FROM inserted);
END;
--Maquinaria
CREATE TRIGGER ValidarFechasMaquinarias
ON Maquinarias
AFTER INSERT, UPDATE
AS
BEGIN
    DELETE FROM Maquinarias
    WHERE FechaInicioRenta > FechaFinalizacionRenta AND MaquinariaID IN (SELECT MaquinariaID FROM inserted);
END;

--Maquinaria Detalles
CREATE TRIGGER ValidarFechasMaquinariaDetalles
ON [Maquinaria Detalles]
AFTER INSERT, UPDATE
AS
BEGIN
    DELETE FROM [Maquinaria Detalles]
    WHERE FechaInicioAsignacion > FechaFinAsignacion AND Maquinaria_DetalleID IN (SELECT Maquinaria_DetalleID FROM inserted);
END;
--Empleados--
CREATE TRIGGER ValidarFechasEmpleados
ON Empleados
AFTER INSERT, UPDATE
AS
BEGIN
    DELETE FROM Empleados
    WHERE FechaNacimiento > FechaContratacion AND EmpleadoID IN (SELECT EmpleadoID FROM inserted);
END;
--Avaluos--
CREATE TRIGGER ValidarFechasAvaluos
ON Avaluos
AFTER INSERT, UPDATE
AS
BEGIN
    DELETE FROM Avaluos
    WHERE FechaInicio > FechaFin AND AvaluoID IN (SELECT AvaluoID FROM inserted);
END;
